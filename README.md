...
| [`libresprite/`](libresprite) | Pixel art & animated sprite editor (layers, onion skinning, tiled drawing, palettes). GPL-2.0, fully free — forked from Aseprite's last GPL commit before Aseprite went source-available/paid. Verified in this repo: downloaded the official v1.2 Linux AppImage, ran it fully headless (no display), and did a real load→scale→export round-trip (8×8 PNG → 32×32 PNG) to confirm it actually works, not just that the binary exists. Source: [LibreSprite/LibreSprite](https://github.com/LibreSprite/LibreSprite) | Needs compiling from source (CMake + Ninja + SDL2/freetype/etc., see `libresprite/INSTALL.md`) or grab a prebuilt release from the [releases page](https://github.com/LibreSprite/LibreSprite/releases) — GUI needs a display, but `--batch` CLI mode (sprite-sheet export, scaling, format conversion) runs headless |
