...
| [`github-ai-tracker/`](github-ai-tracker) | Always-on Node/Express service that tracks a list of GitHub usernames you manage and annotates each new event (pushes, PRs, issues, releases...) with a short Claude-generated "AI expert" insight. REST API + dashboard, optional webhook delivery, deployable to Render via the included `render.yaml`. | `cd github-ai-tracker && npm install && npm start` (see its `README.md` for Render deploy steps) |
