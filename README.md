...
| [`reversi-game/`](reversi-game) | Self-contained Reversi/Othello game (Hebrew RTL UI) — play black vs. a computer opponent with 5 difficulty levels (minimax + alpha-beta pruning, positional/mobility/disc-count heuristics), legal-move hints, undo, and color choice. Local original, no external source. | Open `index.html` directly |
| [`snake-3d-game/`](snake-3d-game) | Neon 3D Snake (Hebrew RTL UI) — Three.js scene with bloom post-processing, smooth interpolated movement, glowing snake/food, particle bursts, synthesized sound effects, keyboard/touch/swipe controls, 3 difficulty levels, and a localStorage high score. Three.js is bundled locally with esbuild (`game.bundle.js`), so it's fully offline — no CDN, no internet needed. Local original, no external source. | Open `index.html` directly |
