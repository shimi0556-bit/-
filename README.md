...
| [`glade-mcp/`](glade-mcp) | MCP server connecting Claude Code (or Cursor/Windsurf) directly to a running Unity or Godot editor — 275 Unity tools / 115 Godot tools (scenes, scripts, physics, UI, gameplay scaffolders), screenshot-based verification, and a free CC0 asset pipeline. Free and local (MIT); only the optional cloud memory/RAG layer needs a paid key. Needs Unity or Godot **running with a GUI** on your own machine — can't run inside this headless repo. Source: [Glade-tool/glade-mcp](https://github.com/Glade-tool/glade-mcp) | Install the Unity package or Godot addon from `glade-mcp/unity-bridge/` or `glade-mcp/godot-bridge/` into your game project, open the editor, then Claude Code auto-connects via the `.mcp.json` at repo root |
| [`arrow/`](arrow) | Game narrative design tool (Godot-based) — 100% visual branching-dialogue/text-adventure editor, VCS-friendly save files, one-click HTML export, CSV (i18n) export. MIT. Verified headless with Godot 4.7.2 in this repo (boots clean, no script errors). Source: [mhgolkar/Arrow](https://github.com/mhgolkar/Arrow) | Open the folder as a project in the Godot 4 editor (needs a GUI to actually use the node editor) |
| [`godot-4-procedural-generation/`](godot-4-procedural-generation) | Procedural generation algorithms and demos for Godot 4 (random walker dungeons, biome/river world maps, modular weapons, infinite asteroid fields). MIT code + CC-BY 4.0 art (credit GDQuest). Verified headless with Godot 4.7.2 in this repo. Source: [gdquest-demos/godot-4-procedural-generation](https://github.com/gdquest-demos/godot-4-procedural-generation) | Open the folder as a project in the Godot 4 editor, run any demo scene under its folder |
| [`libresprite/`](libresprite) | Pixel art & animated sprite editor (layers, onion skinning, tiled drawing, palettes). GPL-2.0, fully free — forked from Aseprite's last GPL commit before Aseprite went source-available/paid. Verified two ways in this repo: (1) downloaded the official v1.2 Linux AppImage and ran it headless, (2) **compiled it from source in this container** (apt deps + a pip-installed CMake 4.4 since Ubuntu 24.04's apt CMake is too old, then `cmake -G Ninja` + `ninja libresprite`, 767/767 targets, clean exit) — both binaries passed the same real load→scale→export round-trip (8×8 PNG → 32×32 PNG). Source: [LibreSprite/LibreSprite](https://github.com/LibreSprite/LibreSprite) | `git submodule update --init --recursive`, install the Linux deps listed in `libresprite/INSTALL.md`, then `mkdir build && cd build && cmake -G Ninja -DCMAKE_BUILD_TYPE=Release .. && ninja libresprite` — needs CMake 4.1+ (`pip install cmake` if your distro's is older). Or skip building: grab a prebuilt release from the [releases page](https://github.com/LibreSprite/LibreSprite/releases). GUI needs a display, but `--batch` CLI mode (sprite-sheet export, scaling, format conversion) runs headless |

Most of these were vendored (copied in, not submoduled) from [hoodini](https://github.com/hoodini) (Yuval Avidani)'s public GitHub repos; `claude-hud`, `gods-eye-view`, `glade-mcp`, `arrow`, `godot-4-procedural-generation`, and `libresprite` are from different, independent authors.

**Godot 4.7.2** is installed at `/usr/local/bin/godot` in this dev environment (headless-capable binary, downloaded from the official [godotengine/godot](https://github.com/godotengine/godot) releases) — used to verify `arrow/` and `godot-4-procedural-generation/` boot without errors. It won't persist to a new container; re-download the same release asset if you need it again here.
