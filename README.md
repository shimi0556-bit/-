...
| [`drive-checker-plus/`](drive-checker-plus) | Chrome extension — a Google Drive toolbar shortcut in the style of Checker Plus for Gmail: popup with recent files & search, desktop notifications on new/changed files, unread badge count. Read-only Drive scope, needs a one-time OAuth client ID (see its README). Original | `chrome://extensions` → Developer mode → Load unpacked → select this folder, then follow `drive-checker-plus/README.md`'s OAuth setup |
