...
| [`chess-game/`](chess-game) | Self-contained browser chess game (vanilla HTML/CSS/JS, vendored [chess.js](https://github.com/jhlywa/chess.js) for rules) — click-to-move, legal-move highlighting, check/checkmate/stalemate detection, captured pieces, move history, undo/reset/flip, and an optional minimax computer opponent. Hebrew RTL UI. | Open `index.html` directly, or serve statically |
