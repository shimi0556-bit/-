...
| [`claude-ui-kit/`](claude-ui-kit) | React component library for building Claude-powered chat UIs — message bubbles with Markdown/code rendering, tool-call cards, typing indicator, auto-resizing input, and a `ChatWindow` + `useClaudeChat` hook. Themeable via CSS variables, light/dark out of the box. Authored in this repo. | `cd claude-ui-kit && npm install && npm run dev:demo` |
