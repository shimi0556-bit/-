...
//! `off`. Custom profiles via `~/.logan/sandbox.toml` or `.grok/sandbox.toml`.
...
/// Load sandbox config from `~/.logan/sandbox.toml` and `.grok/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.logan/sandbox.toml
    let global_path = grok_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.logan/sandbox.toml (additive only)
    let project_path = workspace.join(".logan").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    ...
    let project =
        load_config_file(&workspace.join(".logan").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub fn to_capability_set_with_config(
        ...
        // directories like ~/.logan/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.logan/sandbox.toml or .grok/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
