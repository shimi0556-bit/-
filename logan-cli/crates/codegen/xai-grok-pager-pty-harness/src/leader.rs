...
//! elected leader) plus a fixed leader socket beneath its `LOGAN_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `LOGAN_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared LOGAN_HOME => one leader; the socket lives beneath it so
    ...
    /// The cluster's sessions root: `LOGAN_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".logan").join("sessions")
    }
