...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.logan
    ...
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit LOGAN_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // LOGAN_HOME is set in the test runner's env).
            ("LOGAN_HOME".into(), grok_home),
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("LOGAN_HOME").as_deref(),
            content.home().join(".logan").to_str()
        );
