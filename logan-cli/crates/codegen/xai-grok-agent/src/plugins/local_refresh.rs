...
mod tests {
    ...
    fn refresh_local_install_picks_up_new_agent() {
        ...
        let mut registry = InstallRegistry::empty(home.join(".logan").join("installed-plugins"));
        ...
        let trust = TrustStore::load_from(home.join(".logan").join("trusted-plugins"));
    ...
    fn refresh_keeps_stale_when_legacy_subdir_scope_lost() {
        ...
        let install_dir = home.join(".logan").join("installed-plugins");
