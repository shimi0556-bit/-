...
//!
//! **Trust storage**: `~/.logan/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.logan/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.logan/trusted-plugins` does not exist, returns an empty store.
    ...
    pub fn load() -> Self {
        ...
        // read as the user trust store when neither LOGAN_HOME nor a home dir resolves.
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.logan/trusted-plugins`.
    ...
    /// rewrites `~/.logan/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.logan/trusted-plugins`.
