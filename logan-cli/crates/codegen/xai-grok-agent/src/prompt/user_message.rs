...
//! - `Default` -- the legacy Logan prefix (built by the shell layer).
...
pub enum UserMessageTemplate {
    /// Legacy Logan prefix: `<user_info>` + optional `<git_status>`.
...
pub struct UserMessageContext {
    ...
    /// User-scoped rule files (~/.logan/, ~/.claude/).
    ...
    /// (`~/.logan/projects/<encoded-cwd>/mcps`). Surfaced in
