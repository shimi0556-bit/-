...
impl WorktreeDb {
    ...
    /// Open the default DB at `~/.logan/worktrees.db`.
    ///
    /// Discovers grok home via `$LOGAN_HOME`, falling back to the canonicalized
    /// `$HOME/.logan` (matching `xai_grok_config::grok_home`).
...
pub fn resolve_grok_home() -> Result<PathBuf> {
    ...
    let home = PathBuf::from(std::env::var("HOME").context("neither $LOGAN_HOME nor $HOME is set")?);
    ...
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".logan"))
}

/// Serializes tests that mutate the process-global `LOGAN_HOME` env var so they
...
#[cfg(test)]
static LOGAN_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`LOGAN_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `LOGAN_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `LOGAN_HOME` before `_lock` releases, so the env is correct
...
impl GrokHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = LOGAN_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("grok-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via LOGAN_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until LOGAN_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("LOGAN_HOME");
        unsafe { std::env::set_var("LOGAN_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
    ...
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("LOGAN_HOME", p),
                None => std::env::remove_var("LOGAN_HOME"),
            }
        }
    }
}
