...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$LOGAN_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `grok_home()` OnceLock). A seeded `$LOGAN_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("LOGAN_HOME")
    ...
    /// `$LOGAN_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`LOGAN_HOME` /
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$LOGAN_HOME` even when offline mode was not
...
mod tests {
    ...
    /// `$LOGAN_HOME` env so tests never race the parallel harness.
