...
//! persisted to `~/.logan/tip_cursor.json`.
...
/// Load the cursor from `~/.logan/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.logan/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.logan/tip_cursor.json`, so every session sees the next tip
