...
pub struct UpdateConfig {
    ...
    /// Auth scope key for `~/.logan/auth.json`.
...
/// `~/.logan/bin/logan` symlink target (`../downloads/grok-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `logan update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.logan/downloads`
...
/// Read the cached stable version from `~/.logan/version.json` (sync, for display).
///
...
/// `~/.logan/version.json` (written by the auto-updater):
...
mod tests {
    ...
    fn test_version_from_versioned_binary_name() {
        ...
        assert_eq!(
            version_from_versioned_binary_name("grok-pager-0.1.5-darwin-arm64", "logan-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
