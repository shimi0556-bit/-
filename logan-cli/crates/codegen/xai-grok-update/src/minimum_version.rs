...
//! `max(latest, minimum)`; otherwise the user is asked to run `logan update`.
//!
...
pub(crate) enum MinimumVersionError {
    ...
    #[error(
        "This version of Grok ({current}) is no longer supported. \
         Run `logan update` to install version {minimum} or later."
    )]
    ...
    #[error(
        "Cannot install Grok {target}: the configured minimum is {minimum}. \
         Run `logan update` to install the latest allowed version."
    )]
...
/// Used by `logan update` to keep the install target at or above the pin.
