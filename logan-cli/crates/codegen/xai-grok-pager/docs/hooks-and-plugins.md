...
Logan supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.logan/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `logan plugin marketplace add <source>`)
...
Sources land in `~/.logan/config.toml`:
...
Or in `~/.logan/settings.json` / `~/.claude/settings.json`:
