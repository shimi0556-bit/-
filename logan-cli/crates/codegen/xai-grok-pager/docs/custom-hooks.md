...
2. Create a simple hook file, e.g. `~/.logan/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.logan/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.logan/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
Hooks from `~/.logan/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.logan/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.logan/logs`).
...
Copy them to `~/.logan/hooks/` and customize.
