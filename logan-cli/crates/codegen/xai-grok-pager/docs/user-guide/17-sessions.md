...
Sessions are identified by a unique session ID (a UUIDv7 when Grok generates it; a client may supply its own ID with `-s`) and stored on disk under `~/.logan/sessions/`. Set `LOGAN_HOME` to override the base directory; when it is unset, Grok uses `~/.logan`.
...
Run `logan --resume` without an ID to resume the most recent session for the current directory.
...
List or search sessions from the command line. `logan sessions` requires a subcommand:
...
`logan sessions list` shows sessions for the current working directory, grouped by worktree label. Each row lists the session ID, the creation and update dates, the source status, and the summary. `logan sessions search` combines a local SQLite index with remote results.
...
Resume a session in a fresh worktree with `logan -w -r <session-id>`.
...
The smaller state files -- `summary.json`, `plan.json`, and `signals.json` -- are plain JSON rather than JSONL. JSONL is the source of truth for session content; `logan sessions search` additionally maintains a local SQLite FTS5 index over session titles and prompts for fast keyword search.
