...
| Location | Notes |
|----------|-------|
| `<dir>/.logan/rules/` | Always scanned |
...
1. **Global rules**: `~/.logan/` (applies to all projects)
...
| Path | Purpose |
|------|---------|
| `.logan/config.toml` | Project-scoped MCP servers, plugins, and permission rules (other settings load only from `~/.logan/config.toml`) |
...
Use `logan inspect` to see all loaded project instructions:
...
5. **Version control your rules.** Commit AGENTS.md to the repository so the whole team benefits. User-specific overrides belong in `~/.logan/` (global rules).
