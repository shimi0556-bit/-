...
```bash
# Run with workspace sandbox (read everywhere, write to CWD + temp dirs + ~/.logan/)
grok --sandbox workspace

# Read-only mode (read everywhere, write only to ~/.logan/ + temp dirs)
grok --sandbox read-only

# Most restrictive profile (read CWD + system paths, write CWD + temp dirs + ~/.logan/, no child network)
grok --sandbox strict
```
...
| `workspace`           | Everywhere         | CWD + `~/.logan/` + `/tmp` + `/var/tmp`         | Allowed       | Normal development                |
| `devbox`              | Everywhere         | All top-level dirs except `/data`              | Allowed       | Disposable dev VMs                |
| `read-only`           | Everywhere         | `~/.logan/` + `/tmp` + `/var/tmp`               | Blocked¹      | Exploration, code review          |
| `strict`              | CWD + system paths | CWD + `~/.logan/` + `/tmp` + `/var/tmp`         | Blocked¹      | Untrusted code                    |
...
**workspace** -- The recommended profile for everyday development. The agent can read any file on the system (for understanding dependencies, system libraries, etc.) but can only write to the current working directory, `~/.logan/`, and temp directories (`/tmp`, `/var/tmp`, plus the macOS temp dirs). Network access is allowed for tools like `web_search` and MCP servers.
...
**read-only** -- Use when you want the agent to analyze code without modifying your project files. The agent can read everything but can only write to `~/.logan/` (needed for session persistence) and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

**strict** -- The most restrictive profile, for reviewing untrusted code. The agent can only read files within the current working directory and essential system paths. Writes are limited to CWD, `~/.logan/`, and temp directories. Child-process network access is blocked on Linux (no-op on macOS).

---

## Custom Profiles

Create custom sandbox profiles in `~/.logan/sandbox.toml` (global) or `.grok/sandbox.toml` (per-project):
...
for the life of the session**. When you resume it (`logan --resume <id>`,
`logan --continue`, or `logan -r`), Grok restores that same profile automatically —
...
Sandbox events are logged to `~/.logan/sandbox-events.jsonl` for debugging. Events include:
