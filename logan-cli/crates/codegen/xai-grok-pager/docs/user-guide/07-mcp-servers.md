...
MCP servers are configured in `~/.logan/config.toml` under `[mcp_servers.<name>]` sections.
...
> - `config.toml` — user-level (`~/.logan/config.toml`) **or repo-level**
>   (`.logan/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.logan/config.toml` >
...
By default `logan mcp add` writes to `~/.logan/config.toml` (`--scope user`). Use `--scope project` to write to `.logan/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `logan mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`logan mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
MCP servers can be configured per-project by placing a `.logan/config.toml` in your repository:
...
Grok walks from the current directory up to the git repo root, loading `.logan/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.logan/config.toml` | All projects | Lowest |
| `<repo-root>/.logan/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.logan/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native Grok config | `~/.logan/config.toml`, `.logan/config.toml` | Always on |
...
The Claude and Cursor MCP sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] mcps = false` in `~/.logan/config.toml` or the corresponding environment variable (`GROK_CURSOR_MCPS_ENABLED`, `GROK_CLAUDE_MCPS_ENABLED`). See [Configuration](05-configuration.md#harness-compatibility) for details. Use `logan inspect` to see which MCP servers were loaded and their vendor origin (`[cursor]`, `[claude]`).
...
You must authenticate OAuth-based MCP servers before you can use them. Grok stores the resulting tokens under `~/.logan/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
For stdio servers, Grok captures the process's standard error to `~/.logan/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.logan/logs/mcp/filesystem.stderr.log
```

### Viewing Server Status

Use `logan inspect` to see all loaded MCP servers and their sources:
