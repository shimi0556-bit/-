...
- **`logan dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.logan/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Logan · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Logan's sibling views (
`tasks_pane`):
...
process. The plan's Phase 4 ("supervisor / `logan --bg`") would list
