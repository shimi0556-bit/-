...
Reference for key bindings in the Logan TUI. Bindings are built in and cannot currently be remapped.
...
Simple mode is active by default. To switch to Vim mode, set `vim_mode = true` under `[ui]` in `~/.logan/config.toml`, or toggle it at runtime with `/vim-mode`. See [Configuration](05-configuration.md) for details.
...
> in `~/.logan/config.toml` (or `/vim-mode` to toggle). Arrow keys, `Tab`,
