...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.logan/auth.json` from `GROK_AUTH_JSON` (path) or the
//! developer's `~/.logan/auth.json` so the pager skips device-login when
...
/// Prefer explicit path, else the user's real `~/.logan/auth.json`.
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    if let Some(src) = auth_json_source() {
        ...
        eprintln!("pty_auto_mode: no ~/.logan/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("LOGAN_HOME".into(), grok.display().to_string()),
