...
#
# Auth: GROK_DEPLOYMENT_KEY (takes precedence) or ~/.logan/auth.json from `grok login`.
...
# Read a token from ~/.logan/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_grok_token() {
    local auth_file="$HOME/.logan/auth.json"
    local scope="$1"
...
if [ -n "$GROK_DEPLOYMENT_KEY" ]; then
    ...
    if [ -n "$OIDC_TOKEN" ]; then
        ...
        echo "Auth: using OIDC token from ~/.logan/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.logan/auth.json." >&2
    fi
fi
...
DOWNLOAD_DIR="$HOME/.logan/downloads"
BIN_DIR="${GROK_BIN_DIR:-$HOME/.logan/bin}"
...
if [ "$os" = "windows" ]; then
    ...
    # (default layout: ~/.logan/bin and ~/.logan/downloads are siblings).
...
mkdir -p "$HOME/.logan/completions/bash" "$HOME/.logan/completions/zsh"
"$BIN_DIR/grok" completions bash > "$HOME/.logan/completions/bash/grok.bash" 2>/dev/null || true
"$BIN_DIR/grok" completions zsh  > "$HOME/.logan/completions/zsh/_grok"     2>/dev/null || true
...
CONFIG_FILE="$HOME/.logan/config.toml"
...
if [ -n "$GROK_DEPLOYMENT_KEY" ]; then
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.logan/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.logan/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.logan/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.logan/requirements.toml"
        fi
    fi
fi
...
# Also update shell config so ~/.logan/bin is on PATH for future sessions
...
fish_add_path $HOME/.logan/bin
# <<< grok installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> grok installer >>>
export PATH="$HOME/.logan/bin:$PATH"
fpath=(~/.logan/completions/zsh $fpath)
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    ...
    echo '  export PATH="$HOME/.logan/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use grok from cmd.exe or PowerShell, add %USERPROFILE%\\.logan\\bin to your PATH." >&2
fi
