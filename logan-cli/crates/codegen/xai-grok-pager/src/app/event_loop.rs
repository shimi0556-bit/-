...
pub(crate) async fn run(
    ...
    // Apply initial config (may come from existing ~/.logan/pager.toml).
    ...
    loop {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.logan/config.toml [ui]`
                // rather than `~/.logan/pager.toml`. The watcher only knows
