...
//!
//! Env sandboxing follows this crate's `serial(LOGAN_HOME)` idiom; note
...
//! `GROK_LEADER_SOCKET`, `LOGAN_HOME`) for a real agent's whole lifetime, and
...
struct PagerLeaderCluster {
    ...
    /// agent on the same LOGAN_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(LOGAN_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let env = vec![
            crate::test_util::EnvVarGuard::set("LOGAN_HOME", grok_home.path()),
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same LOGAN_HOME when the next generation's
