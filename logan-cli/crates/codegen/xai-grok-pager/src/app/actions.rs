...
///
/// Both variants write to `~/.logan/config.toml` and route ACP
...
pub enum Effect {
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.logan/config.toml`.
    ...
    /// Persist a typed setting to `~/.logan/config.toml`. On failure,
    ...
    /// registry (`~/.logan/active_sessions.json`).
