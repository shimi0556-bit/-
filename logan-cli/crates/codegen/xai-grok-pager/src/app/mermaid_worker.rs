...
thread_local! {
    ...
    /// `LOGAN_HOME` (whose `grok_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite — PNGs could land in the
    /// real `~/.logan`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_LOGAN_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `LOGAN_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    /// hermetic, with no process-global `LOGAN_HOME` mutation. The `TempDir` lives
