#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn apply_retry_state_pool_402_sets_credit_limit_blocked() {
        ...
        apply_retry_state(
            &RetryState::Failed {
                error_type: "proxy_error".into(),
                message:
                    "API error (status 402 Payment Required): Logan usage balance exhausted"
                        .into(),
            },
            &mut session,
            &mut scrollback,
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `logan logout` /
    /// `logan login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `logan logout` then `logan login`"
                    .into(),
            },
            &mut session,
            &mut scrollback,
