...
/// Shared temp `LOGAN_HOME` for disk-replay tests. `grok_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `LOGAN_HOME`
...
pub(super) fn replay_disk_test_home() -> &'static std::path::Path {
    ...
    HOME.get_or_init(|| {
            ...
            unsafe {
                std::env::set_var("LOGAN_HOME", tmp.path());
            }
            tmp
        })
        .path()
