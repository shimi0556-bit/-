#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.logan` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.logan` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.logan` reads).
    #[test]
