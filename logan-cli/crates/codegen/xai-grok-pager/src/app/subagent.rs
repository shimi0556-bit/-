...
thread_local! {
    static REPLAY_LOGAN_HOME : std::cell::RefCell < Option < std::path::PathBuf >> = const
...
pub(crate) fn set_replay_grok_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_LOGAN_HOME.with(|h| *h.borrow_mut() = home);
}
fn effective_grok_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_LOGAN_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
