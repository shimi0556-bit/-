...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `logan login` to authenticate \
         (or `logan login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         logan login --device-code\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `logan login` on a machine with a browser."
            .to_string()
    }
}
...
pub async fn run_single_turn(
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.logan can't hang exit.
