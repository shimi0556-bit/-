//! `logan plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
/// Typed entry for `logan plugin list --json`. The `status` field acts as a
...
/// Typed entry for `logan plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `logan plugin list`).
        name: String,
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `logan plugin install --help` to get started.");
    } else {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `logan plugin update {}` to update it.",
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `logan plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `logan plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
