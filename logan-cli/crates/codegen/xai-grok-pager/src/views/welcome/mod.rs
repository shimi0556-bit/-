...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Logan** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | Logan Beta [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Logan Beta**  VERSION (left-aligned).
    HeroInline,
}
    ...
    let mut result = match params.auth_state {
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Logan is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that Logan
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Logan may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
