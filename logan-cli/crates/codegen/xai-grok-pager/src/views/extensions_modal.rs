...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    ...
    // Plugin / installed-plugin dirs, under the user grok home (LOGAN_HOME-aware)
    // or a project-scoped `{cwd}/.logan/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User grok home (LOGAN_HOME-aware).
    ...
    // Project hooks
    if source_dir.ends_with("/.logan/hooks") || source_dir.contains("/.logan/hooks/") {
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.logan/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // grok-home branch is LOGAN_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.logan/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.logan/installed-plugins/vendor-abc123/skills");
    ...
    fn local_plugins_source() -> xai_hooks_plugins_types::MarketplaceScanResult {
        xai_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: "/home/user/.logan/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
