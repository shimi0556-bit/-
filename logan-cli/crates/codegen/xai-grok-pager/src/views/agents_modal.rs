...
/// User-level vs project-level config files (`~/.logan` vs `{cwd}/.grok`).
...
/// Union bundled personas with local `~/.logan/personas` and `{cwd}/.logan/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let dirs = [
        (ConfigFileScope::Project, cwd.join(".logan").join("personas")),
...
/// Shared guard: canonical path under `~/.logan/{subdir}` or `{cwd}/.logan/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let project_suffix = std::path::Path::new(".logan").join(subdir);
...
mod tests {
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".logan").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".logan").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.logan/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".logan").join("personas").join("gone.toml");
