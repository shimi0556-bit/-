...
// and installs it to ~/.logan/bin/ using versioned filenames:
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.logan', 'bin');
...
// Set GROK_INSTALL_COMPLETIONS=1 to auto-generate to ~/.logan/completions.
const GROK_PATH = path.join(CANONICAL_DIR, `grok${EXE}`);
if (process.env.GROK_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        const { spawnSync } = require('child_process');
        const completionsDir = path.join(os.homedir(), '.logan', 'completions');
        ...
        console.log('Completions generated to ~/.logan/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
