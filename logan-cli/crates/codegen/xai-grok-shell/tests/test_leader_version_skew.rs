...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".logan").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let control = LeaderClient::connect(
                home.path().join(".logan").join("leader.sock"),
                "grok-pager-update",
                ClientMode::Stdio,
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".logan").join("auth.json");
