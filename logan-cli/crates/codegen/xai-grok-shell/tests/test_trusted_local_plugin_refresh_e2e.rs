...
//! 4. Smoke-validate session JSON under `$LOGAN_HOME/sessions/` after exit.
//!
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _grok_guard = EnvVarGuard::set("LOGAN_HOME", &grok_home);
    ...
    // grok_home() caches the first LOGAN_HOME per process; a separate test
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/LOGAN_HOME via `cmd.env` below; this global env
    ...
    // Smoke: session storage under LOGAN_HOME/sessions has JSON artifacts.
