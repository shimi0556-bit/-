...
//! caller-owned `$LOGAN_HOME`, then inspects `~/.logan/debug/`:
...
//!   `~/.logan/debug/<sessionId>.txt` with first-party content, and does NOT enable
...
//!   `~/.logan/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.logan/debug/` files).
...
/// The per-session firehose directory under a pinned `$LOGAN_HOME`.
fn debug_dir(home: &Path) -> PathBuf {
    home.join(".logan").join("debug")
}

/// List firehose `*.txt` files under `~/.logan/debug` (excluding the `latest.txt`
...
/// Build a headless `grok -p` command with a pinned `$LOGAN_HOME` so the firehose
/// lands under `<home>/.logan/debug`. Firehose env knobs are cleared so the test
...
fn debug_cmd(
    ...
    cmd.env("LOGAN_HOME", home.join(".logan"));
...
/// A live `agent` session writes `~/.logan/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let workdir = git_workdir();
        let home = TempDir::new().expect("create temp home");
        let grok_home = home.path().join(".logan");
        ...
        let client = GrokStdioClient::spawn_with_home_and_env(
            ...
            workdir.path(),
            home,
            &[("GROK_DEBUG_LOG", "1"), ("LOGAN_HOME", &grok_home_str)],
        )
        .await;
        ...
        // file, so `tail -f ~/.logan/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // `~/.logan/logs/sampling.jsonl` is never written — the `--debug`
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.logan/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
