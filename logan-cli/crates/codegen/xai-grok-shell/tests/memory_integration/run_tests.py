...
class IsolatedEnv:
    ...
    The agent subprocess gets a fake $HOME with:
      ~/.logan/auth.json   (copied from real home)
      ~/.logan/memory/     (pre-populated by tests)
      ~/.logan/logs/       (memory.log appears here)
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.logan/config.toml from env1.write_config
