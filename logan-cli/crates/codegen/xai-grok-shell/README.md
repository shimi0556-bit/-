...
Credentials are stored in `~/.logan/auth.json` and persist across sessions. Tokens expire after 7 days; Grok will prompt you to re-authenticate when needed.
...
**3. Run `grok`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.logan/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
4. **stdout** → captured by Grok and saved to `~/.logan/auth.json` as the access token
...
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  ...
  -H "Authorization: Bearer $(jq -r '."https://accounts.x.ai/sign-in".key' ~/.logan/auth.json)" \
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.logan/auth.json` (set by `grok login`)                                                                                                                              |
...
Grok reads configuration from `~/.logan/config.toml`. If the file doesn't exist, Grok uses sensible defaults. You only need to specify values you want to override.
...
- project config: `<repo>/.logan/lsp.json`
- user config: `~/.logan/lsp.json`
...
| --------------------------- | ----- | -------- |
| `./.logan/skills/`           | Local | Highest  |
| `<repo_root>/.logan/skills/` | Repo  | Medium   |
| `~/.logan/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.logan/skills/`) are outside the repo and never filtered.
...
Grok discovers agent definitions from `.grok/agents/` (project), `~/.logan/agents/` (user), and built-in agents. Priority (highest wins):
...
**Combining with `[endpoints]` config:** You can also set endpoints in `~/.logan/config.toml`:
...
MCP servers are configured in `~/.logan/config.toml`:
...
| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.logan/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.logan/config.toml` | This repository   | ↑        |
| `<cwd>/.logan/config.toml`       | Current directory | Highest  |
...
If you also have a `linear` server in `~/.logan/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.grok/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.logan/config.toml`.
...
Memory is stored as Markdown files under `~/.logan/memory/`:
- **Global** (`~/.logan/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.logan/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.logan/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.logan/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Key options under `[memory]` in `~/.logan/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.logan/memory/` for external edits and reindex on search |
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.logan/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.logan/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.logan/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.logan/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.logan/sandbox.toml` (global) or `.grok/sandbox.toml` (per-project):
...
Sandbox events (profile applied, violations) are logged to `~/.logan/sandbox-events.jsonl`
...
- **Skills** — from `.grok/skills/`, `~/.logan/skills/`, plugins, and config paths
...
Sessions are stored under `~/.logan/sessions/`, organized by URL-encoded working directory:
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.logan/config.toml` | Configuration file                                  |
| `~/.logan/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.logan/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.logan/memory/`     | Cross-session memory files and index                |
| `~/.logan/skills/`     | User-scoped skill definitions                       |
| `~/.logan/plugins/`    | User-scoped plugins                                 |
| `~/.logan/agents/`     | User-scoped agent definitions                       |
...
| `GROK_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.logan/debug/<sessionId>.txt`, a path writes that one file |
...
```bash
mkdir -p ~/.logan/completions/bash
grok completions bash > ~/.logan/completions/bash/grok.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.logan/completions/bash/grok.bash" ]] && source "$HOME/.logan/completions/bash/grok.bash"
```
...
```bash
mkdir -p ~/.logan/completions/zsh
grok completions zsh > ~/.logan/completions/zsh/_grok
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.logan/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
```bash
# Per-session debug log (~/.logan/debug/<sessionId>.txt)
grok --debug
...
cat ~/.logan/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.logan/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.logan/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
