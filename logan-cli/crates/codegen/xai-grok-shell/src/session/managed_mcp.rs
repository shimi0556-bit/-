...
mod tests {
    ...
    fn lower_precedence_http_servers_are_blocked_by_toml_name_claims() {
        ...
        std::fs::create_dir_all(cwd.path().join(".logan")).unwrap();
        std::fs::write(
            cwd.path().join(".logan").join("config.toml"),
            r#"
[mcp_servers.github]
...
"#,
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.logan/plugins/team-tool",
            "/home/user/.logan/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.logan/plugins/team-tool/mcp-echo-server.py"]
                );
            }
