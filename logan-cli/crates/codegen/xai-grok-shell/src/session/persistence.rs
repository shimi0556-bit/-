...
/// A feedback entry persisted to `~/.logan/sessions/.../feedback.jsonl`.
///
...
/// Searches across ALL cwd directories under `~/.logan/sessions/`.
///
/// Use `session_exists_for_cwd` instead when the target cwd is known
/// (e.g., the `-r` resume path) to avoid false-positive matches.
/// Find a session by ID across **all** CWD directories under `~/.logan/sessions/`.
///
...
/// Collect MCP server stderr logs from `~/.logan/logs/mcp/` for inclusion in the session archive.
        ...
        StorageMode::Writeback => {
            let auth_manager = auth_manager.ok_or_else(|| {
                io::Error::new(
                    ...
                    "Writeback storage mode requires authentication. Run 'logan login' first.",
                )
            })?;
...
/// Walk `~/.logan/sessions/` and delete files with mtime older than `ttl_days`.
