...
//!   under `~/.logan/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.logan/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.logan/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.logan/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.logan/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.logan/`.
...
/// `CleanAll` mode. Placed under `~/.logan/worktree_pool/<instance_id>/<uuid>/`.
