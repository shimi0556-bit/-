            ...
            Self::NotFound { name } => {
                write!(
                    ...
                    "Plugin \"{name}\" not found.\n\
                     Run `logan plugin list` to see installed plugins."
                )
            }
            Self::NeedsConfirm {
                name,
...
/// When `keep_data` is true, `~/.logan/plugin-data/<id>/` is preserved.
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `logan plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `logan plugin install ./{name}`, or add a \
                     source with `logan plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `logan plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `logan plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
/// `~/.logan/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    ...
    // home separately, so removal still runs from $LOGAN_HOME when no home dir
