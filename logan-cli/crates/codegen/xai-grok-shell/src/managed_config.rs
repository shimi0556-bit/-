...
/// files (TUI tick vs `logan login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `logan setup`; runs retries to completion.
    Standard,
...
struct SyncOutcome {
    ...
    /// `logan setup` doesn't misreport a lock skip as "no config".
    served: bool,
...
/// Post-login hook for `logan login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `logan setup` could install config for.
...
/// `logan setup`); ceasing to serve `fail_closed` rolls back.
...
/// Outcome of the `logan setup` sync. The caller renders it — CLI presentation
...
/// exactly as `logan setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `logan setup` would install them.
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `logan setup` would refuse
...
/// Run the `logan setup` sync for the current principal. The caller must check
