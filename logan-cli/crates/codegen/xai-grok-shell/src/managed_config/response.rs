...
pub enum ManagedConfigError {
    ...
    #[error(
        "Your team sign-in was rejected. It may have expired or lack access. Run `logan login` to sign in again."
    )]
    TeamAuthRejected,
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.logan. Make sure the directory exists and is writable.\n  ({0})"
    )]
