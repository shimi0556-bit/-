...
pub struct LeaderCapabilities {
    ...
    /// (driven by `logan update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `logan update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `logan update` just installed; the leader uses
