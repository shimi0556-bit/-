...
pub async fn resolve_api_key(explicit: Option<&str>, grok_home: &Path) -> Result<String> {
    ...
    Err(anyhow!(
        "no API key: pass --api-key, set XAI_API_KEY, or run `logan login` to populate \
...
async fn non_interactive_auth_key(grok_home: &Path) -> Result<Option<String>> {
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `logan login` to re-authenticate, \
...
mod tests {
    ...
    /// OIDC scope (the same scope `logan login` writes today and
    ...
    ///   the operator's real `~/.logan/auth.json`, the test would read
