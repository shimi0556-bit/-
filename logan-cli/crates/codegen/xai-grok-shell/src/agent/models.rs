...
impl ModelsManager {
    ...
    /// Hot-reload the catalog from `~/.logan/models_cache.json` after an
    ...
    /// When another grok process sharing `~/.logan` (a `--no-leader` run, a
...
/// routing slug (e.g. "Auto" and "Logan" both route to `grok-build`)
...
mod tests {
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            ...
            make_entry_config_with_id(Some("grok-build"), "grok-build", Some("Logan")),
