...
// NOTE: this folder-trust store (`~/.logan/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`xai_grok_agent::plugins::TrustStore` at `~/.logan/trusted-plugins`, plus the
...
/// `~/.logan/config.toml` is dropped when untrusted. This is intended — untrusted
...
mod tests {
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; LOGAN_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because LOGAN_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. LOGAN_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because LOGAN_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("LOGAN_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. LOGAN_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because LOGAN_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("LOGAN_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; LOGAN_HOME-isolated store; GROK_FOLDER_TRUST unset so the
        ...
        std::fs::create_dir_all(home.path().join(".logan").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. LOGAN_HOME-isolated so the trust store is empty;
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. LOGAN_HOME-isolated so the trust store is empty;
        // GROK_FOLDER_TRUST unset so the default-on feature flag applies.
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). LOGAN_HOME-isolated so the trust store is empty.
        use xai_grok_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("LOGAN_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. LOGAN_HOME-isolated (empty store).
        use xai_grok_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("LOGAN_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".logan").join("agents");
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // fail-closed rather than defaulting open). LOGAN_HOME-isolated (empty
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + LOGAN_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. LOGAN_HOME-isolated so the seeded store is the temp one;
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // LOGAN_TEST_VERSION unset so `is_local_build()` is genuinely true.
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        ...
        // discover_plugins/build_for_cwd/reload. LOGAN_HOME-isolated (empty store);
        // GROK_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("LOGAN_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".logan").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // LOGAN_HOME-isolated (empty store); GROK_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("LOGAN_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".logan").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // LOGAN_HOME-isolated so both the folder-trust store and the plugin trust
        ...
        let plugin = tmp.path().join(".logan").join("plugins").join("trustgate");
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // LOGAN_HOME-isolated so the grant writes to a temp store; GROK_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("LOGAN_HOME", home.path());
        let _flag = EnvGuard::unset("GROK_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".logan").join("hooks");
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.logan/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.logan/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        ...
        // A `<cwd>/.logan/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.logan/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.logan/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let grok = tmp.path().join(".logan");
        std::fs::create_dir_all(&grok).unwrap();
        std::fs::write(grok.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        // no LOGAN_VERSION) auto-trusts, so the gate would never engage without this.
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // LOGAN_HOME so the store is empty/seeded in temp. `#[serial]` because both
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".logan").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".logan").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with LOGAN_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. LOGAN_HOME-isolated so the
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. LOGAN_HOME-isolated so the store starts
        // empty; `#[serial]` because LOGAN_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("LOGAN_HOME", home.path());
        let tmp = repo_tmp();
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; LOGAN_HOME-isolated and
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".logan").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".logan").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".logan")).unwrap();
        std::fs::write(tmp.path().join(".logan").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
