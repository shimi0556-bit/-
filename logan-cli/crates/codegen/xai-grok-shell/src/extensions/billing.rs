...
//!
//! Fetches the authenticated user's Logan billing configuration
...
/// Current billing configuration for Logan coding credits.
///
...
///
/// Keeps history to a count + the most recent period so `~/.logan/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data requires auth with grok.com. Run `logan login` to authenticate.",
    )?;
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data requires auth with grok.com. Run `logan login` to authenticate.",
    )?;
