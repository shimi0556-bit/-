//! Built-in files extracted to `~/.logan/` on startup.
...
///
/// These directories under `~/.logan/skills/` will be deleted on startup
...
fn resolve_skill_content(name: &str, raw: &str, grok_home: &std::path::Path) -> String {
    match name {
        ...
        "help" => {
            ...
            raw.replace("~/.logan/", &grok_home_str)
        }
        _ => raw.to_string(),
    }
}

/// Extract bundled files to `~/.logan/` on startup.
///
...
mod tests {
    ...
    async fn help_skill_discovered_by_skill_pipeline() {
        ...
        std::fs::create_dir_all(workspace.join(".logan").join("skills").join("help")).unwrap();
        std::fs::copy(
