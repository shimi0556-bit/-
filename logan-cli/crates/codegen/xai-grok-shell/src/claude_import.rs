...
pub enum ImportScope {
    /// User-level: writes to `~/.logan/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.logan/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.logan/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.logan/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.logan/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.logan/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.logan/config.toml`.
///
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.logan/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.logan/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        ...
        // Hooks are written separately to ~/.logan/hooks/imported-from-claude.json.
    ...
    if !plan.project_items.is_empty() {
        let project_root = find_project_root(cwd);
        let project_path = project_root.join(".logan").join("config.toml");
        ...
        let hooks_dir = project_root.join(".logan").join("hooks");
...
mod tests {
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.logan) don't interfere.
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `LOGAN_HOME` plumbing which is intentionally out of scope.
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.logan/config.toml` may legitimately have the marker
