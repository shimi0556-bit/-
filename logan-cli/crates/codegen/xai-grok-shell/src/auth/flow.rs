...
async fn run_auth_flow_inner(
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `logan login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
/// Unified `logan login` handler for CLI entry points (tui, pager).
///
...
pub async fn run_cli_login_with_options(
    ...
    // supports the device flow. Without this guard, `logan login` on an
    ...
    // config was actually applied; `logan setup` reports the no-config case.
...
/// `logan logout` CLI handler. Calls [`perform_logout`] and formats
...
mod tests {
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `logan login` must use loopback, not device —
    ...
    /// CLI `logan login` passes `on_stderr=None`; stderr must be inherited so
