...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.logan/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.logan/config.toml`,
    ...
    /// (`<cwd>/.logan/config.toml`, `<cwd>/.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.logan/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            // same tick (e.g. `~/.logan/config.toml` AND
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.logan/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.logan/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.logan/config.toml` | `<cwd>`           |
...
/// (`~/.logan/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
mod tests {
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".logan").join("config.toml"), "x = 1").unwrap();
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.logan/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.logan/skills".to_string()]);
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.logan/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.logan/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.logan/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.logan/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
