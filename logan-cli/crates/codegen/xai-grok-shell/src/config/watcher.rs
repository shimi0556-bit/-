...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.logan/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** grok process
    /// sharing the same `~/.logan` (the writer may also be this process;
...
/// Watches `~/.logan/` for `auth.json`, `config.toml`, and `models_cache.json`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.logan/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        // includes `<cwd>/.logan/config.toml` so the loop above
        // watches `<cwd>/.logan/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.logan/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.logan/config.toml` trigger a
    ...
    /// `<cwd>/.logan/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.logan/` does not yet
/// exist at session-open time, the `.grok/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.logan/` followed by a
/// write to `<cwd>/.logan/config.toml` will NOT be observed — the
...
/// expected for a freshly-opened session whose `<cwd>/.logan/` hasn't
...
/// Watches skill directories (`~/.logan/skills/`, `<repo>/.logan/skills/`, etc.)
...
/// recursively: `grok_home` (`~/.logan`, or `$LOGAN_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.logan` alone has `worktrees/`,
...
const HOME_VENDOR_DIRS: &[&str] = &[".logan", ".agents", ".claude", ".cursor"];
...
/// `~/.logan` these exist at startup; later creation is caught on restart).
...
mod tests {
    ...
    fn is_global_config_dir_matches_only_grok_home_and_vendor_dirs() {
        ...
        assert!(!g(&home.join("repo").join(".logan")));
    }

    /// Regression for the ~/.logan inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.logan/worktrees/`) must not
    ...
    /// A write to `<cwd>/.logan/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        let project_grok = cwd.path().join(".logan");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.logan/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.logan/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".logan");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
