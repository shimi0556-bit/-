//! Parse marketplace sources from `~/.logan/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// and `known_marketplaces.json` files under `~/.logan/` and `~/.claude/`.
...
/// instead of `~/.logan`/`~/.claude`. Each root is checked for
