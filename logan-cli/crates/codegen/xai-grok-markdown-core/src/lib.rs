//! Headless markdown analysis sharing Logan's exact `pulldown-cmark` config.
//!
...
/// The exact `pulldown-cmark` option set Logan uses to render markdown.
///
...
/// Parse `text` with Logan's options; count elements and flag structural issues.
