...
pub enum SkillScope {
    /// cwd/.logan/skills
    Local,
    /// repo_root/.logan/skills
    Repo,
    /// ~/.logan/skills
    User,
    /// ~/.logan/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.logan/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.logan/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.logan/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.logan/plugins/infra-plugin",
            "plugin_data": "/root/.logan/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "grok-4",
            "effort": "high",
            "user_invocable": true,
