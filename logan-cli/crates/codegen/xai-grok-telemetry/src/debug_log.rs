...
//!   firehose to `~/.logan/debug/<session_id>.txt` (one file per session), with a
...
pub(crate) enum DebugTarget {
    /// `GROK_DEBUG_LOG=1` → route per session into `<dir>` (`~/.logan/debug`).
...
/// per session into `~/.logan/debug`, an explicit path writes a single file.
///
...
/// `~/.logan/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
