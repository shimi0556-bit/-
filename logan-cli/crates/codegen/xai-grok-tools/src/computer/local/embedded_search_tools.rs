...
//! (release builds, self-extracted to `~/.logan/vendor/<name>-<ver>-<target>`) →
//! `~/.logan/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
// (release pipeline). Self-extracted to `~/.logan/vendor` on first use, mirroring
...
/// Write embedded `bytes` to `~/.logan/vendor/<versioned_name>` (chmod 755) on
...
/// `~/.logan/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
mod tests {
    ...
    /// bytes self-extract under `~/.logan/vendor` and the extracted `bfs` runs.
