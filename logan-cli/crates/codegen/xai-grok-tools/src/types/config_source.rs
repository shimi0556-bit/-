...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.logan/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.logan/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.logan/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.logan/ or ~/.claude/.
    User { path: PathBuf },
