...
/// Returns `~/.logan/worktrees/<repo_slug>` for the given git root.
///
...
/// Resolves the worktree base directory (`~/.logan/worktrees/<repo_name>`)
/// for a given source path, correctly handling grok-managed worktrees.
///
/// When `source_path` is already under `~/.logan/worktrees/<repo>/...`, the
...
/// Walk up from `cwd` (staying within `~/.logan/worktrees/`) to its registered
/// worktree record.
///
...
/// returns `None` (without DB I/O) for paths outside `~/.logan/worktrees/`.
...
///
/// When the source path is already inside `~/.logan/worktrees/<repo>/`, the
...
/// Scan `~/.logan/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    /// Point `LOGAN_HOME` at an isolated tempdir (`resolve_grok_home` re-reads
    ...
    /// and restores `LOGAN_HOME` on drop (before releasing the lock), so the
