...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.logan/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.logan/hooks/`).
    ...
    /// `<cwd>/.logan/lsp.json`: `false` drops them at load, `true` keeps them. The
...
pub enum HookSourceConfig {
    ...
    /// A directory of `*.json` hook files (e.g. `~/.logan/hooks/`).
    Directory(PathBuf),
}
