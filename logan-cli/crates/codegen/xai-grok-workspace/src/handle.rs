...
/// 2. `<grok_home>/workspace`, where `<grok_home>` honours `$LOGAN_HOME` and
///    otherwise falls back to `~/.logan` (see [`xai_grok_config::grok_home`]).
...
pub(crate) mod tests {
    ...
    /// Unique skill names: discovery also reads the dev machine's `~/.logan`.
    #[tokio::test]
