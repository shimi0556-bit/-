...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    ...
    files
        ...
        .map(|mut file| {
            ...
            if file.file_path.contains("/.logan/rules/")
...
/// Load the project config from `<root_cwd>/.logan/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    let config_path = root_cwd.join(".logan").join("config.toml");
...
mod tests {
    ...
    // `~/.logan/skills/`, so on a developer machine the result may be
    ...
    async fn discover_skills_finds_skill_md() {
        ...
        let skills_dir = tmp.path().join(".logan").join("skills").join("my-skill");
    ...
    async fn discover_skills_respects_ignore_config() {
        let tmp = tempfile::tempdir().unwrap();
        let skills_dir = tmp.path().join(".logan").join("skills").join("ignored");
        fs::create_dir_all(&skills_dir).unwrap();
        fs::write(
            skills_dir.join("SKILL.md"),
            "---\nname: ignored\ndescription: Ignored\n---\n",
    ...
    // Discovery also scans the real `~/.logan`, so fixtures use test-unique names.
    #[tokio::test]
    async fn discover_agents_md_strips_rules_frontmatter() {
        let tmp = tempfile::tempdir().unwrap();
        let rules_dir = tmp.path().join(".logan").join("rules");
        ...
        let rule = files
            ...
            .find(|f| {
                f["file_path"]
                    .as_str()
                    .is_some_and(|p| p.ends_with("/.logan/rules/xyzzy-discover-agents-md-test.md"))
            })
    ...
    // from `~/.logan/plugins/`, so tests check for specific plugins.

    #[test]
    fn discover_plugins_finds_manifest_plugin() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".logan").join("plugins").join("test-plugin");
    ...
    fn discover_plugins_json_has_expected_fields() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".logan").join("plugins").join("field-test");
        fs::create_dir_all(plugins_dir.join("skills")).unwrap();
        fs::write(
            plugins_dir.join("plugin.json"),
