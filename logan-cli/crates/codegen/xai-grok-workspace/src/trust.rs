...
//!
//! Persists per-folder trust decisions to `~/.logan/trusted_folders.toml`.
...
//! neither `$LOGAN_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.logan/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.logan/`.
...
/// ROOT (via the `~/.logan/worktrees.db` registry), so every `grok -w` worktree
...
/// sharing `~/.logan/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.logan/trusted-hook-projects` file listed one canonical project
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        // how a cloned repo's own `<repo>/.logan/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
    ...
    /// Point `LOGAN_HOME` at an isolated tempdir and register one grok-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `LOGAN_HOME` on
    ...
    fn register_grok_worktree(
        ...
        let env = LockedTestEnv::lock().set("LOGAN_HOME", &home);
