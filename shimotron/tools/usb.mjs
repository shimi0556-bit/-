...
${QUALITY.map(([h, name, note]) => `    <a class="q" href="Shimotron-Rally.html${h}"><b>${name}</b><span>${note}</span></a>`).join('\n')}
  ...
  <div class="card"><table>
    ...
    <tr><td><kbd>F</kbd></td><td>צילום תמונה (נשמרת בתיקיית ההורדות של המחשב)</td></tr>
  ...
  <div class="card"><ul>
    ...
    <li>התפריט עולה אחרי כמה שניות, והאיים ממשיכים להיבנות ברקע בזמן שבוחרים מצב ורכב (בפינה רואים כמה נשארו).</li>
    <li>בתפריט גוללים למטה (גלגלת העכבר, משטח המגע או האצבע) לבחירת רכב והגדרות, או לוחצים על "בחירת רכב והגדרות ↓".</li>
...
const readme = [
  ...
  '  C / B               מצלמה / מבט לאחור',
  '  F                   צילום תמונה (נשמרת בתיקיית ההורדות)',
  '  AltGr               חזרה למסלול',
  '  Esc / M             עצירה / השתקה',
  '',
